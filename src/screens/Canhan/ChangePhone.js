...
import { CANHAN_PAGE, CHANGE_PHONE_PAGE, PHONE_VERIFICATION_PAGE } from '../../constants/router';
...
class ChangePhonePage extends React.Component {
  ...
  navigationDidFocus = async () => {
    ...
    if (isVerified) {
      const data = await userChangePhone({ token: userIdToken, verificationId: verificationId });
      if (data) {
  ...
  onFormSubmit = async () => {
    try {
      ...
      this.props.navigation.navigate(PHONE_VERIFICATION_PAGE, { route: CHANGE_PHONE_PAGE, phone });
    } catch (error) {
